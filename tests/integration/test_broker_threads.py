"""Integration tests for broker fan-out and concurrent consumer threads."""
...
def test_broker_consumers_run_without_races() -> None:
    ...
    gaze_worker = GazeWorker(
        state,
        settings,
        tracker=MockGazeTracker(x=222.0, y=333.0, confidence=0.87),
...
def test_broker_shutdown_joins_all_workers() -> None:
    ...
    gaze_worker = GazeWorker(
        state,
        settings,
        tracker=MockGazeTracker(x=100.0, y=100.0),
        pipeline=GazePipeline(settings),
    )
    gesture_worker = GestureWorker(
        state,
